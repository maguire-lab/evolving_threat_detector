  ...
  */
  ...
  workflow {
      ...
      if (!params.mode) {
          error "Please specify --mode [make_db|query_db|all]. Use --help for more information."
      }
  ...
  "${projectDir}/assets/schema_input.json"))
          ...
          MAKE_DB_V1(ch_samplesheet_make, ch_amrfinder_input_make, ch_genomes_make,
  ch_proteins_make)
      ...
      if (params.mode == 'query_db' || params.mode == 'all') {
          ...
          if (!input_query_source) {
              error "No input specified for QUERY_DB. Use --input_query or --input"
          }
  ...
  "${projectDir}/assets/schema_input.json"))
          ...
          if (params.mode == 'all') {
              // Use freshly created databases from MAKE_DB
